...
class CFD_Fitness():
    ...
    def __init__(self, debug=False, foampng=False):
        ...
        self.tree = ET.parse('planes/mig21.vsp')
        self.root = self.tree.getroot()
        self.plane = {}
        self.ranges = {}
        self.xml = {}

        self.parts = ['wingsection2', 'vertsection1','horzsection1']

        self.ranges['wingsection2'] = {'Span':(1.5, 5.0),
                                       'TC':(0.0, 2.5), 
                                       'RC':(2.5, 5.0),
                                       'Sweep':(-10.0, 70.0),
                                       'Dihedral':(-10.0,10,0)}

        self.ranges['vertsection1'] = {'Span':(1.5, 2.0),
                                       'Sweep':(0.0, 60.0)}

        self.ranges['horzsection1'] = {'Span':(1.0, 2.0),
                                       'Sweep':(0.0, 60.0),
                                       'Dihedral':(-20.0,20,0)}
        
        wing, vertstab, horzstab = None, None, None
        
        for elem in self.root.getiterator():
            if elem.tag == 'Component':
                if not elem.find("General_Parms") == None:
                    name = elem.find("General_Parms").find("Name")
                    if name.text== "Wing":
                         wing = elem
                    if name.text== "Vertical Stabilizer":
                        vertstab = elem
                    if name.text== "Horizontal Stabilizer":
                        horzstab = elem

        sect_cnt = 0
        for elem in wing.getiterator():
            if elem.tag == "Section":
                name = 'wingsection' + str(sect_cnt)
                self.xml[name] = elem
                sect_cnt += 1

        sect_cnt = 0
        for elem in horzstab.getiterator():
            if elem.tag == "Section":
                name = 'horzsection' + str(sect_cnt)
                self.xml[name] = elem
                sect_cnt += 1

        sect_cnt = 0
        for elem in vertstab.getiterator():
            if elem.tag == "Section":
                name = 'vertsection' + str(sect_cnt)
                self.xml[name] = elem
                sect_cnt += 1
    ...
    def generate_stl(self, idx=None):
        ...
        if self.debug:
            ...
            run_cmd("openscad scripts/genpng.scad --camera=-23,8,-7,25.20,0,120,80 --imgsize=500,300 -o "+planeid+".png")
